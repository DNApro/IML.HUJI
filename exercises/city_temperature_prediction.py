...
def question2_solution(X):
    ...
    fig_avg_temp_IL = px.scatter(IL_df, x="DayOfYear", y="Temp", color="Year",
                                 title="The avg temp in Israel as function of the day of year")
    # fig_avg_temp_IL.write_image("./Q2_average_temp_in_israel_daily.png")
    fig_avg_temp_IL.show()
    fig_std_by_month_IL = px.bar(IL_df.groupby(["Month"], as_index=False).agg(std=("Temp", "std")),
                                 title="The std of temperature in Israel as a function of each month",
                                 x="Month", y="std")
    # fig_std_by_month_IL.write_image("./Q2_std_of_temp_in_israel_monthly.png")
    fig_std_by_month_IL.show()
    return IL_df
...
def question3_solution(X):
    ...
    fig_country_month_grouped.show()
def question4_solution(IL_df):
    train_X, train_y, test_X, test_y = split_train_test(IL_df.DayOfYear, IL_df.Temp)
    losses = []
    for k in range(1, 11):
        losses.append(round(PolynomialFitting(k).fit(train_X, train_y).loss(test_X, test_y), 2))
